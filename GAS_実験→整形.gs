...
function onOpen() {
  ...
  ui.createMenu('📝ANKI化')
    .addItem('①-1 記憶の宮殿_転記', 'fillForAnki')
    .addItem('①-2 記憶の宮殿_HTML化', 'convertSelectedRangeToHtml')
    .addItem('①-3 A記憶の宮殿_CSV化', 'exportSelectedRangeAsCSV')
    .addItem('② イメトレ','copyBDtoFHG_forAnki_withStatus')
    .addToUi();
...
function createAiRequest(text, dateStr, contextInfo, MODEL_NAME, GEMINI_API_KEY) {
  ...
  const prompt = `あなたは「実験結果の管理アシスタント」です。ユーザーが雑に語る「試したこと」を、事実ベースで構造化・評価し、再利用可能な形に整理してください。
    ...
    ▼ If / Then（最重要）
    情報は削らず整理する
    ※セルの内部で改行が必要な箇所には、必ず「  <br>」という文字列を使用してください。
    ※実際の改行（リターンキー）はMarkdownテーブルの構造を壊すため、セル内では絶対に使用しないでください。

    ▼ 方針（重要）
    ・If は「状況（トリガー）」のみを書く（目的は含めない）
    ・Then は「実際に取った行動」のみを書く
    ・どちらも一文で簡潔に書く（20〜40文字目安）
    ・読んだ瞬間に「何をしたか」が分かる粒度にする
    ・説明・補足・意図は書かない（すべて結果に集約）
    ・以下の絵文字は固定ラベルとして必ず使用する（省略・変更禁止）
    ...
    ▼ 構造（追加）
    ・結果の冒頭に「🟦 目的：〇〇」を一文で記載する
    ・その後に「🟩 結果：〇〇」として独り言形式の本文を続ける
    ・「🟦 目的」「🟩 結果」の間は「  <br>」で区切ること

    ▼ 表現ルール
    ・「目的」は簡潔に一文で書く（20〜40文字目安）
    ・「結果」は従来通り、思考の流れがつながる文章にする
    ・不自然な見出し化・箇条書きにはしない（あくまで軽いラベルとして扱う）
    ・一文で完結させず、思考の流れが自然につながる形にする
    ・主観・迷い・納得感の表現を適度に残す
